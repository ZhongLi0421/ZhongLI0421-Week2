...
--%>
<%--todo 2: use c:choose ,c:when c:otherwise to validate username is 'admin' and  password is '000000'--%>
<c:choose>
    <c:when test="${param.username=='admin'&& param.password=='000000'}">
        ...
        <c:redirect url="welcome.jsp">
            <c:param name="username" value="admin"/>  <%--作用：使url栏发生变化--%>
        </c:redirect>
    </c:when>
    <%-- todo 5. use c:otherwise --%>
    <c:otherwise>
        <%-- todo 6:use c:set to set a attribute name message="username password error" in request  --%>
        <c:set var="message" value="username and password error!" scope="request"/>
